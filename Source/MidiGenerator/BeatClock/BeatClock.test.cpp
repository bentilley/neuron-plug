...
SCENARIO("BeatClock - getters and setters")
{
  GIVEN("an instance of a BeatClock")
  {
    ...
    REQUIRE(clock.getWeight() == Approx(1.0f));
    ...
    {
      clock.setWeight(0.286);
      REQUIRE(clock.getWeight() == Approx(0.286));

      clock.setWeight(2.551);
      REQUIRE(clock.getWeight() == Approx(2.551));

      clock.setWeight(1.00035);
      REQUIRE(clock.getWeight() == Approx(1.00035));
    }
  }
}
...
SCENARIO("BeatClock - private methods")
{
  GIVEN("an instance of a BeatClock")
  {
    ...
    GIVEN("BPM and sample rate")
    {
      ...
      {
        bpm = 120.0f;
        REQUIRE(clock.getSamplesPerSubdivision(bpm, sampleRate) == Approx(22050.0f));
      }
      ...
      WHEN("BPM: 114, subdivision: 4")
      {
        bpm = 114.0f;
        clock.setSubdivision(4);
        REQUIRE(clock.getSamplesPerSubdivision(bpm, sampleRate) == Approx(5802.631579f));
      }
      ...
      WHEN("BPM: 99, subdivision: 50")
      {
        bpm = 99.0f;
        clock.setSubdivision(50);
        REQUIRE(clock.getSamplesPerSubdivision(bpm, sampleRate) == Approx(534.545454f));
      }
    }
    ...
    GIVEN("BPM, sample rate, and the time in samples")
    {
      ...
      WHEN("BPM: 120, subdivision: 1, timeInSamples: 2459723749")
      {
        ...
        REQUIRE(clock.getNumberOfNextHit(bpm, timeInSamples, sampleRate) == 111553);
      }
      ...
      WHEN("BPM: 114, subdivision: 4, timeInSamples: 937502")
      {
        ...
        clock.setSubdivision(4);
        REQUIRE(clock.getNumberOfNextHit(bpm, timeInSamples, sampleRate) == 162);
      }
      ...
      WHEN("BPM: 99, subdivision: 50, timeInSamples: 448375")
      {
        ...
        clock.setSubdivision(50);
        REQUIRE(clock.getNumberOfNextHit(bpm, timeInSamples, sampleRate) == 839);
      }
    }
  }
}
